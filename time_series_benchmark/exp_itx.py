...
from models.itransformer import InverseTransformer, InverseTransformerHyper
...
# load config
with open("./configs/itx.yaml", "r") as f:
        ...
        for lr in lrs:
            ...
            for h_idx, horizon in enumerate(horizons):
                ...
                for d in cfg["datasets"]:
                    ...
                    if len(horizons) == len(cfg["params"]["d_hidden"]):
                        d_hidden = cfg["params"]["d_hidden"][h_idx]
                    else:
                        d_hidden = cfg["params"]["d_hidden"][0] # default
                    epochs = cfg["epochs"]
                    if "epochs" in d:
                        epochs = d["epochs"]
                    ...
                    for v in variants:
                        ...
                        # dataloaders
                        dataloader_train = DataLoader(dataset_train, batch_size, shuffle=True, num_workers=8)
                        dataloader_val = DataLoader(dataset_val, batch_size, shuffle=True, num_workers=8)
                        dataloader_test = DataLoader(dataset_test, batch_size, shuffle=False, num_workers=8)
                        dataloaders = [dataloader_train, dataloader_val, dataloader_test]

                        # model/variant
                        if v=="Base":
                            model = InverseTransformer(history, horizon, d_hidden, n_channels)
                        elif v=="Hyper":
                            model = InverseTransformerHyper(history, horizon, d_hidden, d_hyper_hidden, n_input=n_channels, d_embedding=d_embedding)
                        elif v=="HyperE":
                            model = InverseTransformerHyper(history, horizon, d_hidden, d_hyper_hidden, n_input=n_channels, d_embedding=d_embedding, embedding=embedding)
                        ...
                        # save logs
                        df_results.to_parquet("./results/{}_{}_iTX_{}_d_{}_e_{}_s_{}.parquet".format(date, time, variants_str, datasets_str, epochs, seeds_str))

                        # save checkpoint
                        PATH = "./checkpoints/{}_{}_iTX_{}_d_{}_e_{}_s_{}/".format(date, time, variants_str, datasets_str, epochs, seeds_str)
                        os.makedirs(PATH, exist_ok=True)
                        torch.save(model.state_dict(), PATH + "{}-{}_iTX_{}_s-{}_{}_{}_{}_e-{}_L-{}_H-{}_bs-{}_lr-{}_hh-{}_emb-{}.pt".format(
                            date,
                            time,
                            v,
                            seed,
